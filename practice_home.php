...
.side-score .date-submit{
	border-right: 1px solid #000;
	padding-right: 5px;
	font-size: 14px;
}
.side-score .author-name{
	padding-left: 14px;
	font-size: 14px;
}
.side-score a.btn{
...
.side-score a.btn:hover{
	background-color: #2a55c3;
}
...
<?php
	...
	echo '<nav class="navbar fixed-top navbar-expand-lg">
		...
		<img src="Capture2.PNG" width="150px" height="40px" class="d-inline-block align-top" alt="">
			...
			<div class="GiTyN nnn">
				<a class="nav-item nav-link mobile challenge" href="challenge_home.php"><svg class="svg-icon" width="26px" height="26px" fill="#fff" viewBox="0 0 20 20">
				...
				<a class="nav-item nav-link XiOhy challenge" href="challenge_home.php">Challenges<span class="sr-only">(current)</span></a>
			</div>
			<div class="GiTyN">
...
<!--sub navbar -->
	...
	<ul class="nav">
		...
		<li class="nav-item">
		<a class="nav-link challenge_sub_nav" href="#">Practices</a>
		</li>
		<li class="nav-item">
		<a class="nav-link challenge_sub_nav" href="#">Custom Events</a>
		</li>
	</ul>
	</div>
                ...
                <div class="row">
					<div class="col-9">
						<div class="container bg-light">
							<h4 class="text-center mb-5 pt-3">Recently Question</h4>
							...
							$sql = "SELECT * FROM `prac_problem` LIMIT {$offset},{$limit}";
							$result=mysqli_query($conn,$sql);
							$num=mysqli_num_rows($result);

							
							$sql1 = "SELECT * FROM `prac_problem`";
							$result1=mysqli_query($conn,$sql1);
							$num1=mysqli_num_rows($result1);
		
							$total_page = ceil($num1 / $limit);
							if($num>0){
...
?>
										...
										<li>
										<aside>
										<?php echo $score;?><span> Score</span>
										</aside>
										<h5>
											<a href="practice_question.php?title=<?php echo $name;?>"><?php echo $name;?></a>
										</h5>
										<p>
											<span class="date-submit"><?php echo $date;?></span>
											<span class="author-name"><?php echo $author;?></span>
											<a class="btn rounded" href="practice_question.php?title=<?php echo $name;?>">view</a>
										</p>
										</li>
									</ul>
							...
							<?php	}
							}   echo '<ul class="pagination admin-pagination pg-blue">';
									if ($page > 1) {
										echo '<li class="page-item mr-2 ml-2"><a href="practice_home.php?page='.($page - 1).'" class="page-link">Prev</a></li>';
									}
								...
								for ($i=1; $i <= $total_page; $i++) {
									...
									echo '<li class="page-item mr-2 ml-2 '.$active.'"><a class="page-link" href="practice_home.php?page='.$i.'">'.$i.'</a></li>';
								}
								if ($total_page > $page) {
									echo '<li class="page-item mr-2 ml-2"><a href="practice_home.php?page='.($page + 1).'" class="page-link">Next</a></li>';
								}
