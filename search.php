...
<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
       $username=mysqli_escape_string($conn, $_POST['search']);
       ...
       while($row=mysqli_fetch_assoc($result)){
        ...
        echo '
        <div class="container">
        <h3 class="text center row lead my-2 ">Search Results for : <strong><em>'.$row['username'].'</em></strong>
        <a href="index.php"><button class="btn ml-5 row btn-outline-primary my-2 my-sm-0 " type="submit">Back</button>
        </a></h3>
        ...
        <div class="container">
          <h1 class="display-5">'.$row['F_name'].' '.$row['L_name'].'</h1>
          <p class="lead">Username : <strong><em>'.$row['username'].'</em></strong>
          <br>Institution : <strong><em>'.$row['Institution'].'</em></strong>
          <br>Location : <strong><em>'.$row['city'].' '.$row['state'].' '.$row['contry'].'</em></strong>
          <br><a href="profile.php?link='.$row['username'].'"><input class="btn btn-outline-info my-2 my-sm-0" type="submit" value="View Profile"></a>
          </p>
        </div>
      </div>';
