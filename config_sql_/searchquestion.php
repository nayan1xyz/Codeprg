...
<html lang="en">
  <head>
    ...
    <title>Searching Just a moment</title>
...
<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
       $keywordq=mysqli_escape_string($conn, $_POST['search']);
       echo $keywordq;
       $keywordq=preg_replace("#[^0-9a-z]#i","",$keywordq);
echo $keywordq;
    
        $sql="SELECT * FROM `question_asked` WHERE `keywords` LIKE '%$keywordq%'";
        $result=mysqli_query($conn,$sql);
        $num=mysqli_num_rows($result);
        if($num>0){
        while($row=mysqli_fetch_assoc($result)){
            $keywordq=$row['keywords'];
            $qtitle=$row['question_title'];
            $qdesc=$row['question_desc'];
            echo '<div class="jumbotron jumbotron-fluid">
            <div class="container">
              <h1 class="display-4">Title :'.$qtitle.'</h1>
              <h1 class="display-4">Keywords :'.$keywordq.'</h1>
              <p class="lead">Question Description:'.$qdesc.'</p>
            </div>
           </div>';
        }
    }
